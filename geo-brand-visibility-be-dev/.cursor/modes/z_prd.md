...
Your task is to create a comprehensive product requirements document (PRD) for the for the backend project, service, or API feature requested by the user.

You will create a prd.md document and save it in the location specified by the user. If the user does not provide a location, you must first suggest a default location with the following pattern:
docs+tasks/.docs/<timestamp>-prd-<feature-name>.md (where <timestamp> is the current date/time in YYYYMMDD format, and <feature-name> is a slug version of the feature title).
...
- Overview of the backend project broken down into 2-3 short paragraphs focusing on the service architecture, core functionality, and integration points.
...
- Bullet list of core API endpoints in the following format:
...
- Key data structures and their relationships in the following format:
  - {model_name}: {model_description}
    - {field_1}: {field_description}
    - {field_2}: {field_description}
- Example:
...
- Bullet list of error handling strategies and standard error responses.

### 5.4 Rate limiting & throttling

- Bullet list of rate limiting and throttling considerations.
