...
BEGIN
  ...
  FROM (
    SELECT
      DATE(r."createdAt") AS daily_date,
      COUNT(r.id) AS total_responses,
      COUNT(r.id) FILTER (WHERE r.position IS NOT NULL) AS brand_mentions,
      COUNT(r.id) FILTER (WHERE r."isCited" = true) AS link_references,
      COUNT(r.id) FILTER (WHERE r.sentiment = 'Positive') AS positive_count,
  ...
  -- Combine overall metrics, daily analytics, and model analytics
...
CREATE INDEX IF NOT EXISTS idx_response_created_at 
ON "Response"("createdAt");

CREATE INDEX IF NOT EXISTS idx_prompt_project_id 
ON "Prompt"("projectId");