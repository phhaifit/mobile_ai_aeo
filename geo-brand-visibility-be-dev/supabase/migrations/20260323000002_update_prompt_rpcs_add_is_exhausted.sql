-- Replace SELECT p.* with explicit columns so future Prompt table changes don't break RPCs

-- 1. get_prompts_with_latest_analysis_result
DROP FUNCTION IF EXISTS get_prompts_with_latest_analysis_result(UUID, UUID);

CREATE OR REPLACE FUNCTION get_prompts_with_latest_analysis_result(
	p_project_id UUID,
	p_user_id UUID
)
RETURNS TABLE(
...
-- 2. get_prompts_with_latest_analysis_result_by_topic
DROP FUNCTION IF EXISTS get_prompts_with_latest_analysis_result_by_topic(UUID, UUID);

CREATE OR REPLACE FUNCTION get_prompts_with_latest_analysis_result_by_topic(
	p_topic_id UUID,
	p_user_id UUID
)
RETURNS TABLE(
...
-- 3. get_prompts_with_latest_analysis_result_pagination
...
-- 4. get_prompts_with_latest_analysis_result_by_topic_pagination
DROP FUNCTION IF EXISTS get_prompts_with_latest_analysis_result_by_topic_pagination(UUID, UUID, INT, INT, TEXT, "PromptType"[], BOOLEAN);

CREATE OR REPLACE FUNCTION get_prompts_with_latest_analysis_result_by_topic_pagination(
	p_topic_id UUID,
	p_user_id UUID,
    p_limit INT DEFAULT 10,
    p_offset INT DEFAULT 0,
