-- Add revenueModel and customerType to Brand table as arrays
-- A business can combine multiple revenue models and serve multiple customer segments

ALTER TABLE "Brand"
ADD COLUMN IF NOT EXISTS "revenueModel" TEXT DEFAULT NULL,
ADD COLUMN IF NOT EXISTS "customerType" TEXT DEFAULT NULL;

-- Drop all versions of insert_brand to ensure a clean slate
DO $$
DECLARE
    func_record RECORD;
BEGIN
...
CREATE OR REPLACE FUNCTION insert_brand(
    ...
    "revenueModel" text DEFAULT NULL,
...
BEGIN
    ...
    RETURN jsonb_build_object(
        ...
        'revenueModel', insertedBrand."revenueModel",
        'customerType', insertedBrand."customerType",
        'services', insertedServices
    );
END;
$$ LANGUAGE plpgsql;

-- Drop all versions of update_brand and recreate with array fields
DO $$
DECLARE
    func_record RECORD;
BEGIN
